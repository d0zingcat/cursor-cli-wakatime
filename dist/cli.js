...
function getCursorHomeDirectory() {
...
var Dependencies = class {
  ...
  getRequestHeaders() {
    return {
      "User-Agent": "github.com/wakatime/cursor-cli-wakatime"
    };
  }
...
`;
  ...
  async sendRequest(url, options) {
    ...
    const proxy = options?.proxy ? new URL(options.proxy) : void 0;
    ...
    return new Promise(async (resolve3, reject) => {
      ...
      try {
        if (proxy && targetUrl.protocol === "https:") {
          ...
          req = requestModule.request(requestOptions, (response) => resolve3(response));
        }
  ...
  async requestWithRedirects(url, options, redirectsLeft = 5) {
  ...
  async downloadToFile(url, outputFile, options) {
    const response = await this.requestWithRedirects(url, options);
    const statusCode = response.statusCode ?? 0;
  ...
  getCliLocationGlobal() {
    ...
    const found = which.sync(binaryName, { nothrow: true });
    if (found) {
      this.cliLocationGlobal = found;
      this.logger.debug(`Using global wakatime-cli location: ${found}`);
    }
  ...
  installCli(callback) {
    ...
    const zipFile = path3.join(this.resourcesLocation, "wakatime-cli" + this.randStr() + ".zip");
    this.downloadFile(
      url,
      zipFile,
      () => {
  ...
  reportMissingPlatformSupport(osname, architecture) {
    const url = `https://api.wakatime.com/api/v1/cli-missing?osname=${osname}&architecture=${architecture}&plugin=cursor-cli`;
    const proxy = this.options.getSetting("settings", "proxy");
    const noSSLVerify = this.options.getSetting("settings", "no_ssl_verify");
...
var LOG_FILE = import_path.default.join(import_os.default.homedir(), ".wakatime", "cursor-cli-wakatime.log");
var Logger = class {
  constructor(level) {
    this.level = 1 /* INFO */;
...
function isOurHookEntry(entry) {
  ...
  if (command.includes(path6.join("cursor-cli-wakatime", "scripts", "run"))) return true;
  if (/\/_cacache\/tmp\/git-clone[^/'"]+\/scripts\/run/.test(command)) return true;
  return false;
}
function hookEntryMatchesCurrentInstall(entry) {
...
function printStatus() {
  ...
  console.log(`hooks.json: ${hooksPath} (${fs5.existsSync(hooksPath) ? "exists" : "missing"})`);
  console.log(`hook script: ${hookScript} (${fs5.existsSync(hookScript) ? "exists" : "missing"})`);
  console.log(`wakatime-cli: ${deps.getCliLocation()} (${deps.isCliInstalled() ? "exists" : "missing"})`);
  console.log(`wakatime.cfg: ${options.getConfigFile(false)} (${fs5.existsSync(options.getConfigFile(false)) ? "exists" : "missing"})`);
  if (fs5.existsSync(hooksPath)) {
    const config = readHooksConfig(hooksPath);
    for (const eventName of HOOK_EVENTS) {
      const entries = config.hooks[eventName] ?? [];
...
function runDoctor() {
  ...
  if (!fs5.existsSync(options.getConfigFile(false))) {
  ...
  const nodeResult = (0, import_child_process.spawnSync)(process.execPath, ["-v"], { encoding: "utf8" });
